...
2. На Linux portable PostgreSQL уже лежит в offline_deps, Redis собирается локально из исходников.
3. setup.ps1/setup.sh разворачивают окружение локально.
4. Сервер работает по HTTP в доверенной LAN.
5. Секреты генерируются и хранятся локально.
